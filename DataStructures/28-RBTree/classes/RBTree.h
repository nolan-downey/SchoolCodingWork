	...
	private:
		...
		********************************************/
    	...
    	// Step 1 - Return if the root
    		...
    		// Scenario 1 - If the Sibling is Red
				...
				// All the other scenarios are if the sibling is black
				else {
					// Sibling has a red child
					if (sibling->hasRedChild()) {

						// Scenario 2 - The Sibling's red child is the left child
						if (sibling->left != NULL && sibling->left->color == RED) {

    						// Sibling & Sibling's child form left Zig-Zig
								if (sibling->isOnLeft()) {
								...
								// Sibling & Sibling's child form left Zig-Zag
								else {
							...
							// Scenario 3 - The Sibling's red child is the right child
    							...
    							// Sibling & Sibling's child form right Zig-Zag
									if (sibling->isOnLeft()) {
							...
							// Scenario 4 - The sibling has two Black Children
								...
								// Set the sibling's color to red
								sibling->color = RED;
								// If the parent's color is Black, recursively call fixDoubleBlack
								if (parent->color == BLACK)
								fixDoubleBlack(parent);
								// Otherwise, just set the parent's color to Black.
								else
								parent->color = BLACK;
							}
						}
				...
				RBTNode<T>* BSTreplace(RBTNode<T>*& x) {
						//when node have 2 children, recursively call the right successor
